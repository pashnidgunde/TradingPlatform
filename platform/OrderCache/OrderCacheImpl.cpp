...
void OrderCacheImpl::cancelOrdersForSecIdWithMinimumQty(
...
unsigned int
OrderCacheImpl::getMatchingSizeForSecurity(const std::string &securityId) {
  auto buy_range = m_orders.get<SecurityIdSideTag>().equal_range(
      std::make_tuple(securityId, SIDE_BUY));
  auto sell_range = m_orders.get<SecurityIdSideTag>().equal_range(
      std::make_tuple(securityId, SIDE_SELL));

  auto shouldFilter = [](const auto &buy, const auto &sell) {
    return buy.company() == sell.company();
  };

  auto sweeped = [](const auto &order) { return order.qty() == 0; };
  ...
  for (auto buyIter = buy_range.first; buyIter != buy_range.second; buyIter++) {
    auto &buy = *buyIter;
    for (auto sellIter = sell_range.first; sellIter != sell_range.second;
         sellIter++) {
      auto &sell = *sellIter;
