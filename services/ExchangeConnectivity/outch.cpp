...
template <typename T> struct SwapBytes {
  constexpr T operator()(T val) {
    static_assert(std::is_same_v<T, uint8_t> or std::is_same_v<T, uint16_t> or
                  std::is_same_v<T, uint32_t> or std::is_same_v<T, uint64_t>);
...
template <typename T> constexpr T beToSe(T val) {
  if constexpr (std::endian::native == std::endian::big)
    return val;
  return SwapBytes<T>()(val);
}

template <typename T> constexpr T seToBe(T val) {
...
// static_assert(seToBe<int>(0x0102030405060708) == 0x0102030405060708);  // not
// supported
...
struct Stream {
  ...
  std::string_view parked() { return std::string_view(buff, size); }

  void clearParked() { size = 0; }
...
struct Decorator {
  void decorate(uint8_t id, const Stream &stream) {
    ...
    std::cout << " System Event: " << stream.e.sys_events
              << " messages: " << '\n';
    std::cout << " Replaced: " << stream.e.replaced << " messages" << '\n';
    std::cout << " Canceled: " << stream.e.canceled << " messages" << '\n';
    std::cout << " Exceuted: " << stream.e.executed
              << " messages: " << stream.e.executed_shares << " executed shares"
              << '\n';
    std::cout << '\n';
  }
};
...
struct OutchParser {
  ...
  template <typename T> static const T *as(const char *offset) {
  ...
  template <> struct Decoder<Canceled> {
  ...
  const char msgType(const char *msgoffset) {
    auto msg_type_offset =
        msgoffset + sizeof(MsgHeader::ml) + sizeof(MsgHeader::pt);
  ...
  const auto &results() { return _streams; }
...
struct FileReader {
  ...
  std::string_view data() const { return std::string_view(_data.get(), _sz); }
