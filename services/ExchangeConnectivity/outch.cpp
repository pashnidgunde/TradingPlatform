...
struct SwapBytes {
    ...
    {
        static_assert(std::is_same_v<T, uint8_t> or std::is_same_v<T, uint16_t> or std::is_same_v<T, uint32_t> or std::is_same_v<T, uint64_t>);
...
struct OutchParser {
    ...
    OutchParser() = delete;
    OutchParser(const std::string_view& b)
        : _b(b)
    {
    }
    OutchParser(const char* buffer, size_t len)
        : _b(buffer, len)
    {
    }
